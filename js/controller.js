...
function countdown() {
  var interval = setInterval(function () {
    ...
    $('#time').text(Math.floor(minutes / 10) + '' +  minutes % 10 + ':' + Math.floor(second / 10) + '' +  second % 10)
  }, timeInterval);
}
function bounceBall() {
  tt = t;
...
function setGameState(){
  ...
  if(gameState[currentState]['type'] == 'punt_result')$("#gameState").text("Punt");
  if(gameState[currentState]['type'] == 'kickoff_american_football')$("#gameState").text("Kick Off");
}
function setYard(){
